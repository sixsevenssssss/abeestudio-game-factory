...
const _demoBtns = [], _demoToggles = [], _demoSliders = [], _demoTabs = [], _demoInputs = [], _demoCbs = [];
...
function initForms() {
  ...
  const cbDefs=[
    {lk:'cb.sound',  checked:true,  indeterminate:false},
    {lk:'cb.music',  checked:false, indeterminate:false},
    {lk:'cb.notify', checked:false, indeterminate:true },
    {lk:'cb.ads',    checked:true,  disabled:true      },
  ];
  cbDefs.forEach(({lk,checked,indeterminate,disabled})=>{
...
function initTabsSection() {
  ...
  const t3=Tabs({tabs:tabDefs1.map(d=>({id:d.id,label:t(d.key)})),active:'game',disabled:true});g3.appendChild(t3);_demoTabs.push({el:t3,tabDefs:tabDefs1});
  c.append(g1,g2,g3);
}
